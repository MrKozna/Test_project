...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get("SCRT_KEY")
...
def register():
    ...
    if request.method == "POST":
        check_email = User.query.filter_by(email=register_form.email.data).first()
        if not check_email:
            new_user = User(
...
def login():
    ...
    if request.method == "POST":
        ...
        if not check_user:
            ...
            return redirect(url_for("login"))
        elif check_password_hash(check_user.password, login_form.password.data):
